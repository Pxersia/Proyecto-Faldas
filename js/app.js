...
const state = {
  ...
  admin: {name:'Administrador',email:'admin@faldaandco.cl',pwd:'admin123',firstLogin:true},
  orders: [
  ...
  products: [
    {id:'p1',name:'Falda Bohemia Midi',type:'Midi',material:'Lino',price:45990,stock:12,emoji:'🌸',desc:'Falda midi bohemia confeccionada en lino premium con detalles florales bordados. Perfecta para ocasiones especiales y salidas de verano.',sizes:['XS','S','M','L','XL'],colors:['Beige','Blanco','Floral'],featured:true,active:true},
    {id:'p2',name:'Falda Clásica Lápiz',type:'Lápiz',material:'Satén',price:35990,stock:8,emoji:'✨',desc:'Falda lápiz en satén de alta calidad. Silueta elegante que favorece toda figura. Ideal para el trabajo o eventos formales.',sizes:['XS','S','M','L'],colors:['Negro','Azul','Rojo'],featured:false,active:true},
    {id:'p3',name:'Falda Plisada Mini',type:'Plisada',material:'Tul',price:29990,stock:15,emoji:'💫',desc:'Falda mini plisada en tul con volumen perfecto. Un toque juvenil y romántico para cualquier ocasión casual o festiva.',sizes:['XS','S','M','L','XL','XXL'],colors:['Blanco','Rosa','Azul'],featured:true,active:true},
  ]
};
...
function getProductBg(e){const m={'🌸':'#FDF2F8','✨':'#F0F4FF','💫':'#F5F0FF'};return m[e]||'#F5F0E8';}
...
function renderCartItems(){
  ...
  el.innerHTML=state.cart.map(item=>`
    <div class="cart-item">
      ...
      <button class="btn-icon" onclick="removeFromCart('${item.key}')" title="Eliminar">✕</button>
    </div>`).join('');
...
function doRegister(){
  ...
  goPage('dashboard');
  notify('Cuenta creada exitosamente. ¡Bienvenida!');
}
...
function updateNav(){
  ...
  if(state.isAdmin){
    ...
    <div class="cart-badge" id="cartBadge" onclick="toggleCart()">🛍️ <span class="cart-count" id="cartCount">${state.cart.length}</span></div>
  ...
  el.innerHTML=u.addresses.map(a=>`
    <div class="address-card${a.default?' address-default':''}">
      <div>
        ...
        <p style="font-size:.9rem;color:var(--ink2)">${a.city}, ${a.region} — ${a.zip}</p>
        ${a.ref?`<p style="font-size:.8rem;color:var(--ink3);margin-top:.3rem">${a.ref}</p>`:''}
      </div>
      <div style="display:flex;gap:.3rem">
        ${!a.default?`<button class="btn-icon edit" onclick="setDefaultAddress('${a.id}')" title="Establecer como principal">★</button>`:''}
        <button class="btn-icon" onclick="deleteAddress('${a.id}')" title="Eliminar">✕</button>
      </div>
    </div>`).join('');
}
...
function renderPayments(){
  ...
  el.innerHTML=u.payments.map(p=>`
    <div class="payment-card">
      <div class="card-icon">${p.type==='card'?'💳':'🏦'}</div>
      <div style="flex:1">
        <p style="font-weight:500;font-size:.9rem">${p.type==='card'?`Tarjeta ****${p.last4}`:`${p.bank} — ${p.account}`}</p>
        <p style="font-size:.8rem;color:var(--ink3)">${p.type==='card'?p.holder:`RUT: ${p.rut}`}</p>
      </div>
      <button class="btn-icon" onclick="deletePayment('${p.id}')" title="Eliminar">✕</button>
    </div>`).join('');
}
...
function confirmOrder(){
  ...
  notify('¡Pedido confirmado! '+order.id);
}
  ...
  el.innerHTML=state.products.map(p=>`
    <div class="admin-product-card">
      ...
      <div class="actions">
        ...
        <button class="btn-sm" style="color:var(--red);border-color:var(--red)" onclick="deleteProduct('${p.id}')">🗑️</button>
      </div>
    </div>`).join('');
}
